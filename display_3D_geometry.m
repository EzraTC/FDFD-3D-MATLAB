...
if problem_space_display.axis_outside_domain
    ...
    obj.lx = fdfd_domain.min_x;
    obj.ly = fdfd_domain.min_y;
    obj.lz = fdfd_domain.min_z;
    obj.dx = dx; obj.dy = dy; obj.dz = dz;
    object_drawing_functions(obj);
end
...
if problem_space_display.outer_boundaries
    lx = fdfd_domain.min_x;
    ly = fdfd_domain.min_y;
    lz = fdfd_domain.min_z;
    ux = nx*dx+lx;
    uy = ny*dy+ly;
    uz = nz*dz+lz;
...
% draw pml boundaries
if problem_space_display.pml_boundaries
    lx = fdfd_domain.min_x;
    ly = fdfd_domain.min_y;
    lz = fdfd_domain.min_z;
    ux = nx*dx+lx;
    ...
    if strcmp(boundary.type_xn, 'pml')
        lx = lx + abs(dx * boundary.pml_number_of_cells_xn);
    end
    if strcmp(boundary.type_yn, 'pml')
        ly = ly + abs(dy * boundary.pml_number_of_cells_yn);
    end
    if strcmp(boundary.type_zn, 'pml')
        lz = lz + abs(dz * boundary.pml_number_of_cells_zn);
    end
    if strcmp(boundary.type_xp, 'pml')
        ux = ux - abs(dx * boundary.pml_number_of_cells_xp);
    end
    if strcmp(boundary.type_yp, 'pml')
        uy = uy - abs(dy * boundary.pml_number_of_cells_yp);
    end
    if strcmp(boundary.type_zp, 'pml')
        uz = uz - abs(dz * boundary.pml_number_of_cells_zp);
    end
    line([lx lx], [ly ly], [lz uz], 'linewidth', 2, ...
        'color', 'r', 'linestyle', '--');
    line([lx lx], [uy uy], [lz uz], 'linewidth', 2, ...
