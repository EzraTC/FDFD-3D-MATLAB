...
%     problem_space_display.cpml_boundaries = true;
% end
if ~isfield(problem_space_display,'pml_boundaries')
    problem_space_display.pml_boundaries = true;
end
...
xx = xx + fdfd_domain.min_x;
for ind = -1:ny
    yy(:,ind+2,:) = ind * dy;
end
yy = yy + fdfd_domain.min_y;
for ind = -1:nz
    zz(:,:,ind+2) = ind * dz;
end
zz = zz + fdfd_domain.min_z;
...
if exist('number_of_bricks','var')
    for ind = 1:number_of_bricks
        ...
        % convert coordinates to node indices on the FDFD grid
        blx = round((bricks(ind).min_x - fdfd_domain.min_x)/dx)+1; 
        bly = round((bricks(ind).min_y - fdfd_domain.min_y)/dy)+1; 
        blz = round((bricks(ind).min_z - fdfd_domain.min_z)/dz)+1; 

        bux = round((bricks(ind).max_x - fdfd_domain.min_x)/dx)+1; 
        buy = round((bricks(ind).max_y - fdfd_domain.min_y)/dy)+1; 
        buz = round((bricks(ind).max_z - fdfd_domain.min_z)/dz)+1; 
