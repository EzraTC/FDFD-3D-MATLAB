...
vertices_xy_x = vertices_xy_x + fdfd_domain.min_x;

% y coordinate of vertices
y = (meshgrid(0:ny,1:nx+1)*dy).';
y = reshape(y,1,[]).';
vertices_xy_y = reshape(y.',1,[]).';
vertices_xy_y = vertices_xy_y + fdfd_domain.min_y;
...
vertices_yz_y = vertices_yz_y + fdfd_domain.min_y;

% z coordinate of vertices
z = (meshgrid(0:nz,1:ny+1)*dz).';
z = reshape(z,1,[]).';
vertices_yz_z = reshape(z.',1,[]).';
vertices_yz_z = vertices_yz_z + fdfd_domain.min_z;
...
vertices_zx_z = vertices_zx_z + fdfd_domain.min_z;

% x coordinate of vertices
x = (meshgrid(0:nx,1:nz+1)*dx).';
x = reshape(x,1,[]).';
vertices_zx_x = reshape(x.',1,[]).';
vertices_zx_x = vertices_zx_x + fdfd_domain.min_x;
...
for i=size(animation,2):-1:1
    ...
    for j=size(animation(i).plane_cut,2):-1:1
        if animation(i).plane_cut(j).type == 'xy';
            animation(i).plane_cut(j).position_index = round((animation(i).plane_cut(j).position-fdfd_domain.min_z)/dz)+1;
            x = vertices_xy_x;
            y = vertices_xy_y;
            z = vertices_xy_y*0+(animation(i).plane_cut(j).position_index-1) * dz + fdfd_domain.min_z;
            animation(i).plane_cut(j).vertices = [x y z];
            animation(i).plane_cut(j).faces = faces_xy;
            nc = nz;
        end
        if animation(i).plane_cut(j).type == 'yz';
            animation(i).plane_cut(j).position_index = round((animation(i).plane_cut(j).position-fdfd_domain.min_x)/dx)+1;
            y = vertices_yz_y;
            z = vertices_yz_z;
            x = vertices_yz_z*0+(animation(i).plane_cut(j).position_index-1) * dx + fdfd_domain.min_x;
            animation(i).plane_cut(j).vertices = [x y z];
            animation(i).plane_cut(j).faces = faces_yz;
            nc = nx;
        end
        if animation(i).plane_cut(j).type == 'zx';
            animation(i).plane_cut(j).position_index = round((animation(i).plane_cut(j).position-fdfd_domain.min_y)/dy)+1;
            z = vertices_zx_z;
            x = vertices_zx_x;
            y = vertices_zx_x*0+(animation(i).plane_cut(j).position_index-1) * dy + fdfd_domain.min_y;
            animation(i).plane_cut(j).vertices = [x y z];
...
for i=1:size(animation,2)
    if animation(i).enable
         ...
         axis equal;
         view(20,30);
         axis([fdfd_domain.min_x fdfd_domain.max_x ...
             fdfd_domain.min_y fdfd_domain.max_y ...
             fdfd_domain.min_z fdfd_domain.max_z]);
         axis off;
