...
for ind = 1:number_of_sampled_electric_fields
    is = round((sampled_electric_fields(ind).x - fdfd_domain.min_x)/dx) + 1;
    js = round((sampled_electric_fields(ind).y - fdfd_domain.min_y)/dy) + 1;
    ks = round((sampled_electric_fields(ind).z - fdfd_domain.min_z)/dz) + 1;

    sampled_electric_fields(ind).is = is;
    sampled_electric_fields(ind).js = js;
    sampled_electric_fields(ind).ks = ks;

    % Single-frequency phasor placeholder (complex scalar or 1×1)
