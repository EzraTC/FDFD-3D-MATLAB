...
for ind = 1:number_of_voltage_sources
    is = round((voltage_sources(ind).min_x - fdfd_domain.min_x)/dx)+1;
    js = round((voltage_sources(ind).min_y - fdfd_domain.min_y)/dy)+1;
    ks = round((voltage_sources(ind).min_z - fdfd_domain.min_z)/dz)+1;
    ie = round((voltage_sources(ind).max_x - fdfd_domain.min_x)/dx)+1;
    je = round((voltage_sources(ind).max_y - fdfd_domain.min_y)/dy)+1;
    ke = round((voltage_sources(ind).max_z - fdfd_domain.min_z)/dz)+1;
...
for ind = 1:number_of_current_sources
    is = round((current_sources(ind).min_x - fdfd_domain.min_x)/dx)+1;
    js = round((current_sources(ind).min_y - fdfd_domain.min_y)/dy)+1;
    ks = round((current_sources(ind).min_z - fdfd_domain.min_z)/dz)+1;
    ie = round((current_sources(ind).max_x - fdfd_domain.min_x)/dx)+1;
    je = round((current_sources(ind).max_y - fdfd_domain.min_y)/dy)+1;
    ke = round((current_sources(ind).max_z - fdfd_domain.min_z)/dz)+1;
...
for ind = 1:number_of_resistors
    is = round((resistors(ind).min_x - fdfd_domain.min_x)/dx)+1;
    js = round((resistors(ind).min_y - fdfd_domain.min_y)/dy)+1;
    ks = round((resistors(ind).min_z - fdfd_domain.min_z)/dz)+1;
    ie = round((resistors(ind).max_x - fdfd_domain.min_x)/dx)+1;
    je = round((resistors(ind).max_y - fdfd_domain.min_y)/dy)+1;
    ke = round((resistors(ind).max_z - fdfd_domain.min_z)/dz)+1;
    resistors(ind).is = is;
    resistors(ind).js = js;
    resistors(ind).ks = ks;
    resistors(ind).ie = ie;
...
for ind = 1:number_of_inductors
    is = round((inductors(ind).min_x - fdfd_domain.min_x)/dx)+1;
    js = round((inductors(ind).min_y - fdfd_domain.min_y)/dy)+1;
    ks = round((inductors(ind).min_z - fdfd_domain.min_z)/dz)+1;
    ie = round((inductors(ind).max_x - fdfd_domain.min_x)/dx)+1;
    je = round((inductors(ind).max_y - fdfd_domain.min_y)/dy)+1;
    ke = round((inductors(ind).max_z - fdfd_domain.min_z)/dz)+1;
    inductors(ind).is = is;
    inductors(ind).js = js;
    inductors(ind).ks = ks;
    inductors(ind).ie = ie;
...
for ind = 1:number_of_capacitors
    is = round((capacitors(ind).min_x - fdfd_domain.min_x)/dx)+1;
    js = round((capacitors(ind).min_y - fdfd_domain.min_y)/dy)+1;
    ks = round((capacitors(ind).min_z - fdfd_domain.min_z)/dz)+1;
    ie = round((capacitors(ind).max_x - fdfd_domain.min_x)/dx)+1;
    je = round((capacitors(ind).max_y - fdfd_domain.min_y)/dy)+1;
    ke = round((capacitors(ind).max_z - fdfd_domain.min_z)/dz)+1;
...
for ind = 1:number_of_diodes
    is = round((diodes(ind).min_x - fdfd_domain.min_x)/dx)+1;
    js = round((diodes(ind).min_y - fdfd_domain.min_y)/dy)+1;
    ks = round((diodes(ind).min_z - fdfd_domain.min_z)/dz)+1;
    ie = round((diodes(ind).max_x - fdfd_domain.min_x)/dx)+1;
    je = round((diodes(ind).max_y - fdfd_domain.min_y)/dy)+1;
    ke = round((diodes(ind).max_z - fdfd_domain.min_z)/dz)+1;
    diodes(ind).is = is;
    diodes(ind).js = js;
    diodes(ind).ks = ks;
    diodes(ind).ie = ie;
